...
import { diff, evaluateAttributes, stringToHTML } from './diffing.js';
...
class WebComponent extends HTMLElement {
    ...
    _unsubscribeTranslator;

    constructor() {
        super();
        this._unsubscribeTranslator = this.translator.observable$.subscribe(() => this._rerender(false));
        this.init();
        if (this._useShadowDOM()) {
            this.attachShadow({ mode: 'open' });
            this._loadStylesOnce().then(() => this._rerender());
        } else {
            this._rerender();
        }
    }
    ...
    disconnectedCallback() {
        ...
        if (this._unbind)
            this._unbind();
        if (this._unsubscribeTranslator)
            this._unsubscribeTranslator();
    }
     ...
     * @version 1.0.0
     */
    subscribe(queryOrElement, event, callback) {
        const elem = typeof queryOrElement === 'string'
            ? this._getDOM().querySelector(queryOrElement) : queryOrElement;
        if (elem) {
    ...
    subscribeAll(query, event, callback) {
        const elems = this._getDOM().querySelectorAll(query);
        elems.forEach(elem => elem.addEventListener(event, callback));
        elems.forEach(elem => this._subscriptions.push({ elem, event, callback }));
    }
     ...
     * @version 1.0.0
     */
    _rerender(callAfterViewInit = true) {
        this._unsubscribe();
        if (this._unbind)
            this._unbind();
    ...
    _forceRerender() {
        ...
        const node = stringToHTML(this.render());
        evaluateAttributes(node, this);
        this._getDOM().innerHTML = node.innerHTML;
        // this._getDOM().appendChild(node);
        this._unbind = this.bind();
